...
fn test_desktop_entry_created() {
    ...
    let archive_path = tar_dir.path().join("dummy.tar");
    ...
    cmd.env("HOME", temp_home.path())
        .arg(&archive_path)
        .arg("--user"); // force user-level install

    // 5. Assertions
    cmd.assert()
        .success()
        .stdout(predicate::str::contains("Desktop entry created"))
